...
func (h *Handler) configImport(w http.ResponseWriter, r *http.Request) {
	...
	err = h.Store.Update(func(c *config.Config) error {
		...
		if mode == "replace" {
			...
			if incoming.CurrentInputFile.Enabled != nil {
				next.CurrentInputFile.Enabled = incoming.CurrentInputFile.Enabled
			}
			if incoming.CurrentInputFile.MinChars > 0 {
				next.CurrentInputFile.MinChars = incoming.CurrentInputFile.MinChars
			}
