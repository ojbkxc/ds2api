...
> **关于本地工具与原生搜索的共存**
>
> 项目内置了本地 `web_search` 和 `web_fetch` 工具，用于增强非搜索模型的联网能力。对于自带原生搜索的模型（`deepseek-v4-flash-search` / `deepseek-v4-flash-search-nothinking`），为避免两个搜索工具同时存在导致模型选择困难（只输出思考不调用工具），项目会**自动跳过本地 `web_search` 工具注入**，仅保留 `web_fetch` 工具。
>
> | 模型类型 | 原生搜索 | 本地 web_search | 本地 web_fetch |
> | --- | --- | --- | --- |
> | `deepseek-v4-flash` | 无 | 注入 | 注入 |
> | `deepseek-v4-flash-search` | 有 | 跳过 | 注入 |
> | `deepseek-v4-pro` | 无 | 注入 | 注入 |
> | `deepseek-v4-vision` | 无 | 注入 | 注入 |
>
> 之所以保留 `web_fetch`：原生搜索只能搜索，无法直接抓取用户指定的 URL 内容。当用户发送链接要求分析时，模型需要 `web_fetch` 来完成。
...
当托管账号在请求过程中产生失败记录时，系统会自动将该账号标记为禁用状态，防止持续使用已失效的账号导致更多失败。仅以下错误会触发自动禁用：
...
禁用的账号可通过 Admin API 或 WebUI 管理台手动重新启用。该机制确保问题账号被及时发现并隔离，降低对整体服务的影响。

### DeepSeek 客户端认证

DS2API 内部使用 Android App 风格的请求头与 DeepSeek API 通信，关键参数：

| 参数 | 值 | 说明 |
| --- | --- | --- |
| `os` | `android` | 操作系统标识 |
| `x-client-platform` | `android` | 客户端平台 |
| `x-client-version` | `2.2.0` | 客户端版本 |
| `device_id` | `deepseek_to_api` | 固定设备标识 |
| `User-Agent` | Chrome 149/150 Android 池 | 模拟 Android Chrome 浏览器 |

登录时携带 `x-client-bundle-id`、`accept-language` 和 `x-client-timezone-offset` 等头，确保与 DeepSeek Android App 的请求行为一致。
...
DS2API 对不同模型采用不同的会话与上下文管理策略，以在保持对话连续性的同时避免 prompt 膨胀。

### Session 池模型隔离

DeepSeek 上游在 Session 创建时绑定 `model_type`（`default`/`expert`/`vision`），此后同一 Session 只能用于该类型的模型。DS2API 内部的 Session 池以 `accountID:modelType` 为键进行隔离，确保 flash、pro、vision 三种模型的 Session 互不共享，避免跨模型路由错误（如 vision 请求被错误路由到 pro 模型）。

### v4-pro 会话复用

`deepseek-v4-pro`（Expert 模式）不支持文件上传，DeepSeek 通过 `parent_message_id` 维护完整会话上下文。当会话复用时（`parentMessageID > 0`），下游已持有完整历史，再发送完整消息列表会造成重复发送。因此项目会**自动剥离历史消息**，仅保留系统提示词和最新一条用户消息，通过 `StripHistoryForSessionReuse()` 实现。

### v4-pro 新会话创建

当 v4-pro 会话超出上限需要新建会话时，新会话在 DeepSeek 侧没有上下文。此时需要保留最近若干轮对话以便模型理解背景，但不能保留太多导致 prompt 爆炸。项目通过 `TrimHistoryForNewSession(20)` 实现，保留最近 20 轮用户/助手对话（跳过压缩后的工具输出）。

### 上下文压缩

当 prompt token 数超过配置的上下文窗口阈值时，DS2API 会进行三级渐进式压缩，仅作用于工具输出内容：

| 级别 | 触发条件 | 策略 |
| --- | --- | --- |
| **Snip** | 超过窗口 60% | 裁剪过长的工具结果段落，保留头尾 |
| **Prune** | 超过窗口 80% | 将旧工具输出替换为摘要占位符 |
| **Compact** | 超过窗口 90% | 截断到上下文窗口上限 |

压缩通过 `contextcompression.GlobalCompressor` 在构建 prompt 时自动执行，不会影响已存储的对话历史。对于 v4-pro 等不支持文件上传的模型，上下文压缩在 `TrimHistoryForNewSession` 之前执行，确保只保留必要内容。
...
DS2API 使用 DeepSeek 原生标记格式构建 prompt，确保模型能正确理解角色边界和工具上下文：

```
<|begin▁of▁sentence|>
<|System|>系统提示词内容<|end▁of▁instructions|>
<|User|>用户消息内容
<|Assistant|>助手回复内容<|end▁of▁sentence|>
<|Tool|>工具返回内容<|end▁of▁toolresults|>
<|Assistant|>
```
...
| `<|System|>` / `<|end▁of▁instructions|>` | 系统提示词边界 |
| `<|User|>` | 用户消息 |
| `<|Assistant|>` / `<|end▁of▁sentence|>` | 助手回复边界 |
| `<|Tool|>` / `<|end▁of▁toolresults|>` | 工具返回边界 |

### 输出完整性保护

构建 prompt 时，系统会自动在最前面插入一条随机化的输出完整性 guard 系统消息（7 种措辞变体 + 4 位随机数），用于防止模型模仿或回显上游上下文中的乱码、损坏、重复或畸形的片段。约 30% 概率跳过 guard 注入，进一步降低指纹识别风险。

### Prompt 随机化

DS2API 在 prompt 构建层实施了多层随机化，以降低被 DeepSeek 上游通过固定模式检测代理请求的风险：

| 随机化层 | 变体数 | 说明 |
| --- | --- | --- |
| Output integrity guard | 7 种 × 10000 | 输出完整性保护的措辞变体，末尾追加 4 位随机数 |
| Thinking injection | 12 种 × 10000 | 混入正式技术风格和口语化风格（"Think step by step" 等），末尾追加随机数 |
| Tool call 格式标题 | 6 种 × 10000 | TOOL CALL FORMAT / FUNCTION CALLING / API CALL SYNTAX 等 |
| Tool call 提醒语 | 5 种 × 10000 | Remember / Note / Important / Reminder / Key point 等 |
| Tool call 规则 | 4 条 × 各 3 种 | wrapper 包裹 / CDATA 字符串 / 禁止 markdown / 参数名约束 |
| Web search 指南 | 4 种 × 10000 | 不同措辞但功能完全一致的搜索指导 |
| Web fetch 指南 | 4 种 × 10000 | 不同措辞但功能完全一致的抓取指导 |
| 历史转录标题 | 10 种 × 10000 | 对话文件标题随机化 |
| 历史转录摘要 | 12 种 × 10000 | 对话文件摘要随机化 |
| 消息分隔符 | 8 种 × 10000 | 对话轮次分隔格式随机化 |
| 角色标签 | 每角色 8 种 | CLIENT / USER / HUMAN / VISITOR 等 |
| 工具描述模板 | 6 种 × 10000 | 工具 schema 展示格式随机化 |
| 文件名前缀 | 12 种 | tools_ / functions_ / methods_ 等 |
| 延续提示词 | 16 种 × 10000 | 文件上传后的上下文延续提示 |
| 本地工具可用短语 | 6 种 × 10000 | "You have access to these tools:" 等 |

所有随机化使用 `crypto/rand` 密码学安全随机源，确保不可预测。

### 本地工具注入

对于支持本地工具的模型（`deepseek-v4-flash`、`deepseek-v4-pro`、`deepseek-v4-vision`），系统会在构建 prompt 时自动注入 `web_search` 和 `web_fetch` 工具的描述、DSML 调用格式说明、以及使用指南。对于有原生搜索的模型（`deepseek-v4-flash-search`），仅注入 `web_fetch`，避免两个搜索工具冲突。详见 [模型支持](#模型支持) 中的工具共存说明。
...
- **容器镜像发布**：仅推送到 GHCR（`ghcr.io/ojbkxc/ds2api`）
