...
> **关于本地工具与原生搜索的共存**
>
> 项目内置了本地 `web_search` 和 `web_fetch` 工具，用于增强非搜索模型的联网能力。对于自带原生搜索的模型（`deepseek-v4-flash-search` / `deepseek-v4-flash-search-nothinking`），`GetModelConfig` 返回 `search=true`，此时项目会**完全跳过所有本地工具注入**（包括 `web_search` 和 `web_fetch`），让模型仅使用原生搜索。
>
> 之前尝试过只跳过 `web_search` 保留 `web_fetch`，但实际测试发现原生搜索 + 本地 `web_fetch` 并存仍会导致模型困惑（只输出思考不调用任何工具 → 触发 `Upstream account hit a rate limit`），因此改为全面跳过。
>
> | 模型类型 | `search` 标记 | 本地工具 |
> | --- | --- | --- |
> | `deepseek-v4-flash` | `false` | 全部注入（web_search + web_fetch） |
> | `deepseek-v4-flash-search` | `true` | 全部跳过 |
> | `deepseek-v4-flash-search-nothinking` | `true` | 全部跳过 |
> | `deepseek-v4-pro` | `false` | 全部注入 |
> | `deepseek-v4-vision` | `false` | 全部注入 |
>
> 跳过逻辑在 `InjectLocalToolsIntoPrompt`、`MergeLocalToolNames`、`MergeLocalToolsWithClientTools` 三个函数中统一实现，Claude 适配器同步处理。
...
**工具循环保护**：当模型调用本地工具（如 `web_fetch`）时，工具执行结果会以 `role: "tool"` 消息加入请求。`StartCompletion` 在调用 `StripHistoryForSessionReuse` 前会先通过 `HasToolMessages()` 检查是否存在工具消息 — 如果存在则跳过剥离，确保工具结果被保留在 prompt 中供模型继续处理。这样避免了工具循环中 tool results 被误剥离导致模型反复重试工具调用的问题。
...
构建 prompt 时，系统会自动在最前面插入一条 `Output integrity guard` 系统消息，用于防止模型模仿或回显上游上下文中的乱码、损坏、重复或畸形的片段。

### 本地工具注入

对于支持本地工具的模型（`deepseek-v4-flash`、`deepseek-v4-pro`、`deepseek-v4-vision`），系统会在构建 prompt 时自动注入 `web_search` 和 `web_fetch` 工具的描述、DSML 调用格式说明、以及使用指南。对于 `search=true` 的模型（`deepseek-v4-flash-search` / `deepseek-v4-flash-search-nothinking`），**所有本地工具全部跳过**，模型仅使用原生搜索。详见 [模型支持](#模型支持) 中的工具共存说明。
